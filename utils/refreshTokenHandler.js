...
function _RefreshTokenHandler(prisma) {
    return async function (req, res) {
        ...
        if (!token) return res.status(401).json({ status: "NO_TOKEN", message: "Refresh token required" });
        ...
        try {
            ...
            if (storedToken.device !== device || storedToken.ip !== ip) {
                return res.status(403).json({ status: "DEVICE_MISMATCH", message: "Token used from different device/IP" });
            }

            // Verify token
            jwt.verify(token, process.env.REFRESH_SECRET, async (err, user) => {
                if (err) return res.status(403).json({ status: "TOKEN_EXPIRED", message: "Refresh token expired" });
